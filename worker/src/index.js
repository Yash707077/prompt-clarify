...
// A second, independent limit keyed on IP address.
//
// WHY BOTH
// installId is generated by the browser, so anyone can clear their storage,
// open a private window, or just send a new random id with curl and get a
// fresh 15. That was tolerable when only the extension existed. A public web
// page makes the bypass trivial, so we also count per IP, which a user cannot
// change from inside the page.
//
// Set slightly higher than PER_USER_DAILY so that a household or office
// sharing one connection is not blocked by a single heavy user.
...
export default {
  async fetch(request, env) {
    ...
    const byIp = await bump(env.LIMITS, `ip:${ip}:${day}`, PER_IP_DAILY);
    if (!byIp.allowed) {
      return json(
        {
          error: "Daily limit reached for this network. Try again tomorrow.",
          reason: "ip_limit",
        },
        429,
      );
